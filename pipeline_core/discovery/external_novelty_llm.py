...
- LEXICAL IDENTITY ALIGNMENT: whenever prior_art_identity_terms is non-empty, every non-empty predicted_observation and falsification_condition MUST include at least one prior_art_identity_terms entry verbatim as a branch-identity label. This requirement aligns the structured draft with the deterministic branch-specific sanitizer; it does not authorize adding synonyms, mechanisms, variables, directions, thresholds, regimes, or scientific propositions.
- Do not satisfy this rule by changing prior_art_identity_terms to a weaker, broader, or more convenient phrase. Choose prior_art_identity_terms from the atomic scientific branch first, then reuse that already-selected identity text in the specification fields.
...
class InstructorOpenAICompatibleExternalNoveltyBackend:
    ...
    def review_claim(
        ...
        lines = [
            ...
            f"distinguishing_terms: {claim.distinguishing_terms!r}",
            'The structured fields above are claim specification only, not prior-art evidence. They specify the relation to assess; they do not establish that relation. Do not infer missing scientific links from separately mentioned components.',
            "",
