...
@runtime_checkable
class AxisInferenceBackend(Protocol):
    backend_name: str
    model_name: str
...
class InstructorOpenAICompatibleAxisInferenceBackend:
    """Structured discovery-axis inference review.

    This backend evaluates only. Hypothesis repair remains the
    responsibility of the synthesis runtime.
    """

    backend_name = (
        "instructor_openai_compatible_axis_inference_critic"
    )
    ...
    def _get_client(self) -> Any:
        ...
        if not self.api_key:
            raise RuntimeError(
                f"No API key available. Set "
                f"{self.api_key_env} or pass api_key explicitly."
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Axis inference critic backend requires "
    ...
    def review(
        ...
        context = {
            **self.telemetry_context,
            "pipeline":
                "discovery_axis_hypothesis",
            "stage":
                "axis_inference_critic",
            "call_kind":
                "review",
        }
