...
@runtime_checkable
class ReframeDraftBackend(Protocol):
    backend_name: str
    model_name: str

    def generate(self, prompt: ScientificReframePrompt) -> ReframeDraftGeneration: ...


class InstructorOpenAICompatibleReframeBackend:
    ...
    def _get_client(self) -> Any:
        ...
        try:
            ...
            raise RuntimeError(
                "Scientific reframing requires installed 'openai' and 'instructor' packages."
            ) from exc
    ...
    def generate(self, prompt: ScientificReframePrompt) -> ReframeDraftGeneration:
        ...
        draft, event = run_instructor_structured_call(
            ...
            response_model=ScientificReframeBatchDraft,
            messages=[
            ...
            telemetry_context={
                **self.telemetry_context,
                ...
                "stage": prompt.operator_id.lower(),
                "call_kind": "scientific_reframe_generation",
            },
            semantic_components={
                "operator_id": prompt.operator_id,
            },
        )
        if not isinstance(draft, ScientificReframeBatchDraft):
            draft = ScientificReframeBatchDraft.model_validate(draft)
