...
class ProspectiveDecompositionProvenanceCaseExecutionPlanV4(StrictModel):
    case_id: Literal["P26", "P27", "P28"]
    source_task_id: str
    ...
    regeneration_reentry_root: str
    regeneration_lineage_root: str
    regeneration_decomposition_sidecar_filename: Literal[
        "claim_decomposition.sanitization_audit.json"
    ] = "claim_decomposition.sanitization_audit.json"
    regeneration_decomposition_sidecar_relative_glob: Literal[
        "lineage/*/claim_decomposition.sanitization_audit.json"
    ] = "lineage/*/claim_decomposition.sanitization_audit.json"
    regeneration_decomposition_sidecar_schema: Literal[
    ...
    def validate_case(
        self,
    ) -> "ProspectiveDecompositionProvenanceCaseExecutionPlanV4":
        ...
        if self.initial_e2e_argv[:3] != expected_initial_prefix:
            raise ValueError("v4 diagnostic initial argv uses unexpected runner")
        if "--stop-after-initial-semantic" not in self.initial_e2e_argv:
            raise ValueError("v4 diagnostic initial argv lacks semantic cut point")
        ...
        if observed_forbidden:
            raise ValueError(
                "v4 diagnostic initial argv carries forbidden flags: "
        ...
        if self.downstream_campaign_argv_base[:3] != expected_downstream_prefix:
            raise ValueError(
                "v4 diagnostic downstream argv uses unexpected campaign runner"
            )
        if "--semantic-review" in self.downstream_campaign_argv_base:
            raise ValueError(
                "v4 diagnostic downstream base argv must defer semantic-review"
            )
        if "--allow-dirty-worktree" in self.downstream_campaign_argv_base:
            raise ValueError(
                "v4 diagnostic execution plan must not authorize dirty V_post"
            )
        if "--regeneration-unit-freeze" not in self.downstream_campaign_argv_base:
            raise ValueError(
                "v4 diagnostic downstream argv lacks regeneration freeze"
            )
        if "--save-prompts" not in self.downstream_campaign_argv_base:
            raise ValueError(
                "v4 diagnostic downstream argv must preserve prompts"
            )
        ...
        downstream_root = Path(
            self.downstream_campaign_output_root
        ).expanduser().resolve()
        expected_reentry = downstream_root / "04_regeneration_reentry"
        if Path(self.regeneration_reentry_root).expanduser().resolve() != expected_reentry:
            raise ValueError("v4 diagnostic regeneration re-entry root mismatch")
        expected_lineage = expected_reentry / "lineage"
        if Path(self.regeneration_lineage_root).expanduser().resolve() != expected_lineage:
            raise ValueError("v4 diagnostic regeneration lineage root mismatch")
        return self


class ProspectiveDecompositionProvenanceExecutionPlanV4(StrictModel):
    ...
    cases: list[ProspectiveDecompositionProvenanceCaseExecutionPlanV4]
    case_ids: list[str]
    ...
    sidecar_discovery_contract_frozen_before_execution: Literal[True] = True
    ...
    source_id_coverage_observed_before_plan_freeze: Literal[False] = False
    exact_source_rebind_recovery_observed_before_plan_freeze: Literal[
        False
    ] = False
    ...
    case_order_fixed_p26_to_p28: Literal[True] = True
    ...
    def validate_plan(
        self,
    ) -> "ProspectiveDecompositionProvenanceExecutionPlanV4":
        expected = ["P26", "P27", "P28"]
        if self.case_ids != expected:
            raise ValueError(
                "v4 diagnostic execution-plan case IDs must be P26-P28"
            )
        if [row.case_id for row in self.cases] != expected:
            raise ValueError(
                "v4 diagnostic execution-plan cases must be ordered P26-P28"
            )
        if len({row.source_task_id for row in self.cases}) != 3:
            raise ValueError(
                "v4 diagnostic execution-plan source task IDs must be unique"
            )
        if any(row.scientific_validation_authority for row in self.cases):
            raise ValueError(
                "v4 diagnostic case cannot carry scientific validation authority"
            )
        ...
        if observed_sha != expected_sha:
            raise ValueError(
                "prospective decomposition provenance execution-plan v4 SHA mismatch"
            )
        if observed_id != (
            "prospective_decomposition_provenance_execution_plan_v4:"
            + expected_sha[:20]
        ):
            raise ValueError(
                "prospective decomposition provenance execution-plan v4 ID mismatch"
            )
        return self
...
def _downstream_campaign_argv_base(
    *,
    ...
    paths = {
        ...
        "campaign_root": str(
            run / "prospective_decomposition_provenance_v4"
        ),
    }
    argv = [
        "python",
...
def materialize_downstream_argv_v4(
    case: ProspectiveDecompositionProvenanceCaseExecutionPlanV4,
) -> list[str]:
    """Materialize only the frozen semantic-review file-existence policy."""
...
def build_prospective_decomposition_provenance_execution_plan_v4(
    *,
    campaign_freeze: ProspectiveDecompositionProvenanceFreezeV4,
    ...
    if (
        ...
        raise ValueError(
            "v4 diagnostic campaign/regeneration-unit freeze ID mismatch"
        )
    if (
        ...
        raise ValueError(
            "v4 diagnostic campaign/regeneration-unit freeze SHA mismatch"
        )
    if (
        ...
        raise ValueError(
            "v4 diagnostic campaign/regeneration-unit freeze file SHA mismatch"
        )

    if campaign_freeze.scientific_validation_authority:
        raise ValueError(
            "v4 diagnostic campaign must not carry scientific validation authority"
        )
    if not campaign_freeze.engineering_diagnostic_only:
        raise ValueError("v4 diagnostic campaign must be engineering-only")
    ...
    for task in campaign_freeze.tasks:
        ...
        reentry_root = campaign_root / "04_regeneration_reentry"
        lineage_root = reentry_root / "lineage"
        ...
        cases.append(
            ProspectiveDecompositionProvenanceCaseExecutionPlanV4(
                ...
                regeneration_reentry_root=str(reentry_root.resolve()),
                regeneration_lineage_root=str(lineage_root.resolve()),
    ...
    body = {
        ...
        "sidecar_discovery_contract_frozen_before_execution": True,
        ...
        "source_id_coverage_observed_before_plan_freeze": False,
        "exact_source_rebind_recovery_observed_before_plan_freeze": False,
        ...
        "case_order_fixed_p26_to_p28": True,
    ...
    return ProspectiveDecompositionProvenanceExecutionPlanV4(
        **body,
        plan_id=(
            "prospective_decomposition_provenance_execution_plan_v4:"
            + digest[:20]
