...
CaseIdV6 = Literal["P34", "P35", "P36", "P37", "P38"]
DesignAxisV6 = Literal[
    "core_shell_geometry",
    "mechanical_strain",
    "crystallinity_defect",
    "solution_environment",
    "stochastic_dynamics",
]
ComparisonStageV6 = Literal["INITIAL_VPRE", "REGENERATION_REENTRY"]
ComparisonPairV6 = Literal[
...
CaseAccountingStatusV6 = Literal[
    "CLAIMS_COLLECTED",
...
class ProspectiveAtomicAdmissibilityTaskV6(StrictModel):
    case_id: CaseIdV6
    design_axis: DesignAxisV6
...
class ProspectiveAtomicAdmissibilitySpecV6(StrictModel):
    schema_version: Literal[
        "prospective-atomic-admissibility-spec-v6"
    ] = "prospective-atomic-admissibility-spec-v6"
    ...
    tasks: list[ProspectiveAtomicAdmissibilityTaskV6] = Field(
        min_length=5,
        max_length=5,
    )
    ...
    tasks_defined_before_p34_p38_execution: Literal[True] = True
    ...
    infrastructure_and_model_policy_only_inherited_from_v5: Literal[True] = True
    ...
    def validate_spec(self) -> "ProspectiveAtomicAdmissibilitySpecV6":
        expected_cases = ["P34", "P35", "P36", "P37", "P38"]
        expected_axes = [
            "core_shell_geometry",
            "mechanical_strain",
            "crystallinity_defect",
        ...
        if [row.case_id for row in self.tasks] != expected_cases:
            raise ValueError("v6 source tasks must be exactly P34-P38")
        if [row.design_axis for row in self.tasks] != expected_axes:
            raise ValueError("v6 design-axis order mismatch")
        families = [row.relation_family for row in self.tasks]
        if len(families) != len(set(families)):
            raise ValueError("v6 relation families must be unique")
        surfaces = [
            (
        ...
        if len(surfaces) != len(set(surfaces)):
            raise ValueError("v6 source-task surfaces must be unique")
        if len(set(self.semantic_roots)) != len(self.semantic_roots):
            raise ValueError("semantic_roots must be unique")
        return self


class FrozenProspectiveAtomicAdmissibilityTaskV6(StrictModel):
    case_id: CaseIdV6
    task_id: str
    ...
    def validate_task(
        self,
    ) -> "FrozenProspectiveAtomicAdmissibilityTaskV6":
        body = self.model_dump(mode="json")
        observed_id = body.pop("task_id")
        observed_sha = body.pop("task_sha256")
        expected_sha = _sha256_json(body)
        if observed_sha != expected_sha:
            raise ValueError("v6 task SHA mismatch")
        if observed_id != (
            "prospective_atomic_admissibility_v6_task:"
            + expected_sha[:20]
        ):
            raise ValueError("v6 task ID mismatch")
        return self


class ProspectiveAtomicAdmissibilityFreezeV6(StrictModel):
    schema_version: Literal[
        "prospective-atomic-admissibility-freeze-v6"
    ] = "prospective-atomic-admissibility-freeze-v6"
    ...
    tasks: list[FrozenProspectiveAtomicAdmissibilityTaskV6]
    case_ids: list[str]
    task_count: Literal[5] = 5
    design_axis_counts: dict[str, int]
    downstream_campaign_stage_order: list[str]

    source_tasks_frozen_before_p34_p38_execution: Literal[True] = True
    model_policy_frozen_before_p34_p38_execution: Literal[True] = True
    p34_p38_outputs_observed_before_freeze: Literal[False] = False
    ...
    def validate_freeze(
        ...
        expected = ["P34", "P35", "P36", "P37", "P38"]
        if self.case_ids != expected:
            raise ValueError("v6 freeze case IDs must be P34-P38")
        if [row.case_id for row in self.tasks] != expected:
            raise ValueError("v6 frozen task order must be P34-P38")
        if len({row.task_id for row in self.tasks}) != 5:
            raise ValueError("v6 frozen task IDs must be unique")
        if self.downstream_campaign_stage_order != list(CAMPAIGN_STAGE_ORDER):
            raise ValueError("v6 downstream stage order mismatch")
        ...
        if dict(sorted(expected_axis_counts.items())) != dict(
            ...
            raise ValueError("v6 design-axis counts mismatch")
        if any(count != 1 for count in self.design_axis_counts.values()):
            raise ValueError("each v6 design axis must appear once")
        ...
        expected_sha = _sha256_json(body)
        if observed_sha != expected_sha:
            raise ValueError("v6 freeze SHA mismatch")
        if observed_id != (
            "prospective_atomic_admissibility_freeze_v6:"
            + expected_sha[:20]
        ):
            raise ValueError("v6 freeze ID mismatch")
        return self


class ProspectiveAtomicAdmissibilityCaseExecutionPlanV6(StrictModel):
    case_id: CaseIdV6
    source_task_id: str
    ...
    def validate_case(
        self,
    ) -> "ProspectiveAtomicAdmissibilityCaseExecutionPlanV6":
        if self.initial_e2e_argv[:3] != [
            "python",
            "-m",
            "scripts.discovery.run_dac_discovery_e2e",
        ]:
            raise ValueError("v6 initial argv uses unexpected runner")
        if "--stop-after-initial-semantic" not in self.initial_e2e_argv:
            raise ValueError("v6 initial argv lacks semantic cut point")
        if "--overwrite-run" in self.initial_e2e_argv:
            raise ValueError("v6 initial argv must not overwrite run")
        if self.downstream_campaign_argv_base[:3] != [
            "python",
            "-m",
            "scripts.discovery.run_pre_n10_prospective_campaign_v1",
        ]:
            raise ValueError("v6 downstream argv uses unexpected runner")
        if "--semantic-review" in self.downstream_campaign_argv_base:
            raise ValueError("v6 downstream base argv must defer review")
        if "--allow-dirty-worktree" in self.downstream_campaign_argv_base:
            raise ValueError("v6 downstream argv must not allow dirty worktree")
        if "--save-prompts" not in self.downstream_campaign_argv_base:
            raise ValueError("v6 downstream argv must preserve prompts")
        return self


class ProspectiveAtomicAdmissibilityExecutionPlanV6(StrictModel):
    schema_version: Literal[
        "prospective-atomic-admissibility-execution-plan-v6"
    ] = "prospective-atomic-admissibility-execution-plan-v6"
    ...
    cases: list[ProspectiveAtomicAdmissibilityCaseExecutionPlanV6]
    case_ids: list[str]
    case_count: Literal[5] = 5

    source_tasks_and_models_frozen_before_execution: Literal[True] = True
    p34_p38_outputs_observed_before_plan_freeze: Literal[False] = False
    ...
    case_order_fixed_p34_to_p38: Literal[True] = True
    later_case_adaptation_allowed: Literal[False] = False
    failed_or_terminal_case_replacement_allowed: Literal[False] = False
    result_conditioned_route_changes_allowed: Literal[False] = False
    second_regeneration_allowed: Literal[False] = False
    ...
    def validate_plan(
        self,
    ) -> "ProspectiveAtomicAdmissibilityExecutionPlanV6":
        expected = ["P34", "P35", "P36", "P37", "P38"]
        if self.case_ids != expected:
            raise ValueError("v6 execution case IDs must be P34-P38")
        if [row.case_id for row in self.cases] != expected:
            raise ValueError("v6 execution cases must be ordered P34-P38")
        if len({row.source_task_id for row in self.cases}) != 5:
            raise ValueError("v6 execution task IDs must be unique")
        body = self.model_dump(mode="json")
        observed_id = body.pop("plan_id")
        observed_sha = body.pop("plan_sha256")
        expected_sha = _sha256_json(body)
        if observed_sha != expected_sha:
            raise ValueError("v6 execution-plan SHA mismatch")
        if observed_id != (
            "prospective_atomic_admissibility_execution_plan_v6:"
            + expected_sha[:20]
        ):
            raise ValueError("v6 execution-plan ID mismatch")
        return self


class ProspectiveAtomicAdmissibilityComparisonCaseContractV6(StrictModel):
    case_id: CaseIdV6
    source_task_id: str
    source_task_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    ...
    def validate_paths(
        self,
    ) -> "ProspectiveAtomicAdmissibilityComparisonCaseContractV6":
        ...
        if Path(self.regeneration_reentry_report_path).expanduser().resolve() != (
            ...
            raise ValueError("v6 reentry report path mismatch")
        if Path(self.regeneration_lineage_root).expanduser().resolve() != (
            reentry / "lineage"
        ):
            raise ValueError("v6 lineage root mismatch")
        return self


class ProspectiveAtomicAdmissibilityComparisonCollectorFreezeV6(StrictModel):
    schema_version: Literal[
        "prospective-atomic-admissibility-comparison-collector-freeze-v6"
    ] = "prospective-atomic-admissibility-comparison-collector-freeze-v6"
    ...
    cases: list[ProspectiveAtomicAdmissibilityComparisonCaseContractV6]
    case_ids: list[str]
    case_count: Literal[5] = 5
    ...
    collector_frozen_before_p34_p38_execution: Literal[True] = True
    p34_p38_outputs_observed_before_collector_freeze: Literal[False] = False
    ...
    def validate_freeze(
        ...
        expected = ["P34", "P35", "P36", "P37", "P38"]
        if self.case_ids != expected:
            raise ValueError("v6 collector case IDs must be P34-P38")
        if [row.case_id for row in self.cases] != expected:
            raise ValueError("v6 collector cases must be ordered P34-P38")
        body = self.model_dump(mode="json")
        observed_id = body.pop("freeze_id")
        observed_sha = body.pop("freeze_sha256")
        expected_sha = _sha256_json(body)
        if observed_sha != expected_sha:
            raise ValueError("v6 collector freeze SHA mismatch")
        if observed_id != (
            "prospective_atomic_admissibility_comparison_collector_freeze_v6:"
            + expected_sha[:20]
        ):
            raise ValueError("v6 collector freeze ID mismatch")
        return self


class ProspectiveAtomicAdmissibilityClaimComparisonV6(StrictModel):
    case_id: CaseIdV6
    stage: ComparisonStageV6
...
class ProspectiveAtomicAdmissibilityCaseAccountingV6(StrictModel):
    case_id: CaseIdV6
    case_in_denominator: Literal[True] = True
    campaign_final_status: str
    accounting_status: CaseAccountingStatusV6
    ...
    def validate_counts(
        self,
    ) -> "ProspectiveAtomicAdmissibilityCaseAccountingV6":
        if self.total_claim_comparison_count != (
            self.initial_claim_comparison_count
            + self.regeneration_claim_comparison_count
        ):
            raise ValueError("v6 case comparison count mismatch")
        if self.accounting_status == "TERMINAL_BEFORE_INITIAL_VPRE":
            if self.total_claim_comparison_count != 0:
                raise ValueError(
                    "v6 terminal case cannot carry claim comparisons"
                )
        return self


class ProspectiveAtomicAdmissibilityComparisonReportV6(StrictModel):
    schema_version: Literal[
        "prospective-atomic-admissibility-comparison-report-v6"
    ] = "prospective-atomic-admissibility-comparison-report-v6"
    ...
    cases: list[ProspectiveAtomicAdmissibilityCaseAccountingV6]
    claims: list[ProspectiveAtomicAdmissibilityClaimComparisonV6]
    ...
    def validate_report(
        self,
    ) -> "ProspectiveAtomicAdmissibilityComparisonReportV6":
        expected = ["P34", "P35", "P36", "P37", "P38"]
        if self.case_ids != expected:
            raise ValueError("v6 report case IDs must be P34-P38")
        if [row.case_id for row in self.cases] != expected:
            raise ValueError("v6 report cases must be ordered P34-P38")
        if self.claim_comparison_count != len(self.claims):
            raise ValueError("v6 report claim count mismatch")
        if sum(row.total_claim_comparison_count for row in self.cases) != len(
            self.claims
        ):
            raise ValueError("v6 case/claim accounting mismatch")
        body = self.model_dump(mode="json")
        observed_id = body.pop("report_id")
        observed_sha = body.pop("report_sha256")
        expected_sha = _sha256_json(body)
        if observed_sha != expected_sha:
            raise ValueError("v6 comparison report SHA mismatch")
        if observed_id != (
            "prospective_atomic_admissibility_comparison_report_v6:"
            + expected_sha[:20]
        ):
            raise ValueError("v6 comparison report ID mismatch")
        return self


def default_p34_p38_tasks() -> list[ProspectiveAtomicAdmissibilityTaskV6]:
    rows = [
        (
            "P34",
            "core_shell_geometry",
            "metal_shell_thickness_outer_surface_field_confinement",
            "metal shell thickness",
            "electromagnetic near-field confinement at the outer shell surface",
            (
                "How does metal shell thickness relate to electromagnetic "
                "near-field confinement at the outer shell surface in "
                "core-shell SERS nanoparticles?"
            ),
        ),
        (
            "P35",
            "mechanical_strain",
            "substrate_tensile_strain_lspr_shift",
            "substrate tensile strain",
            "localized surface plasmon resonance wavelength shift",
            (
                "How does substrate tensile strain relate to localized surface "
                "plasmon resonance wavelength shift in flexible SERS substrates?"
            ),
        ),
        (
            "P36",
            "crystallinity_defect",
            "grain_boundary_density_plasmon_damping",
            "metal grain-boundary density",
            "plasmon damping rate",
            (
                "How does metal grain-boundary density relate to plasmon "
                "damping rate in polycrystalline SERS nanostructures?"
            ),
        ),
        (
            "P37",
            "solution_environment",
            "ionic_strength_aggregation_kinetics",
            "solution ionic strength",
            "nanoparticle aggregation kinetics",
            (
                "How does solution ionic strength relate to nanoparticle "
                "aggregation kinetics in colloidal SERS systems?"
            ),
        ),
        (
            "P38",
            "stochastic_dynamics",
            "hotspot_residence_time_sers_blinking_frequency",
            "analyte residence time at an electromagnetic hotspot",
            "temporal SERS blinking frequency",
            (
                "How does analyte residence time at an electromagnetic hotspot "
                "relate to temporal SERS blinking frequency in single-molecule "
                "SERS measurements?"
            ),
        ),
    ]
    return [
        ProspectiveAtomicAdmissibilityTaskV6(
            case_id=case_id,
            design_axis=axis,
...
def build_p34_p38_spec_from_v5_infrastructure(
    *,
    infrastructure_freeze: ProspectiveAtomicAdmissibilityFreezeV5,
    campaign_root: str,
) -> ProspectiveAtomicAdmissibilitySpecV6:
    if infrastructure_freeze.case_ids != ["P29", "P30", "P31", "P32", "P33"]:
        raise ValueError("v6 infrastructure source must be P29-P33 v5")
    if len(infrastructure_freeze.tasks) != 5:
        raise ValueError("v6 infrastructure source must contain five tasks")
    ...
    for task in infrastructure_freeze.tasks[1:]:
        for field in invariant_fields:
            if getattr(task, field) != getattr(first, field):
                raise ValueError("v6 infrastructure is non-uniform for " + field)
    ...
    if overlap:
        raise ValueError("v6 relation families overlap prior cohorts: " + repr(overlap))
    if prior_surfaces & new_surfaces:
        raise ValueError("v6 task surfaces overlap prior cohorts")

    return ProspectiveAtomicAdmissibilitySpecV6(
        campaign_name="P34_P38_atomic_admissibility_v6",
...
def build_prospective_atomic_admissibility_freeze_v6(
    *,
    spec: ProspectiveAtomicAdmissibilitySpecV6,
    ...
    body = {
        "schema_version": "prospective-atomic-admissibility-freeze-v6",
        ...
        "source_tasks_frozen_before_p34_p38_execution": True,
        "model_policy_frozen_before_p34_p38_execution": True,
        "p34_p38_outputs_observed_before_freeze": False,
    ...
    digest = _sha256_json(body)
    return ProspectiveAtomicAdmissibilityFreezeV6(
        **body,
        freeze_id=(
            "prospective_atomic_admissibility_freeze_v6:"
            + digest[:20]
...
def _downstream_argv(
    ...
    paths = {
        ...
        "campaign_root": str(run / "prospective_atomic_admissibility_v6"),
    }
    argv = [
        "python",
        "-m",
...
def materialize_downstream_argv_v6(
    case: ProspectiveAtomicAdmissibilityCaseExecutionPlanV6,
) -> list[str]:
...
def build_prospective_atomic_admissibility_execution_plan_v6(
    *,
    campaign_freeze: ProspectiveAtomicAdmissibilityFreezeV6,
    ...
    body = {
        "schema_version": "prospective-atomic-admissibility-execution-plan-v6",
        ...
        "p34_p38_outputs_observed_before_plan_freeze": False,
        ...
        "case_order_fixed_p34_to_p38": True,
        "later_case_adaptation_allowed": False,
        "failed_or_terminal_case_replacement_allowed": False,
        "result_conditioned_route_changes_allowed": False,
        "second_regeneration_allowed": False,
    ...
    digest = _sha256_json(body)
    return ProspectiveAtomicAdmissibilityExecutionPlanV6(
        **body,
        plan_id=(
            "prospective_atomic_admissibility_execution_plan_v6:"
            + digest[:20]
...
def build_prospective_atomic_admissibility_comparison_collector_freeze_v6(
    *,
    execution_plan: ProspectiveAtomicAdmissibilityExecutionPlanV6,
    ...
    if repository_tracked_worktree_dirty:
        raise ValueError("v6 collector freeze requires clean tracked worktree")
    cases = []
    ...
    body = {
        ...
        "collector_frozen_before_p34_p38_execution": True,
        "p34_p38_outputs_observed_before_collector_freeze": False,
    ...
    digest = _sha256_json(body)
    return ProspectiveAtomicAdmissibilityComparisonCollectorFreezeV6(
        **body,
        freeze_id=(
            "prospective_atomic_admissibility_comparison_collector_freeze_v6:"
            + digest[:20]
...
def _collect_claim_set_v6(
    *,
...
) -> list[ProspectiveAtomicAdmissibilityClaimComparisonV6]:
    ...
    if str(audit.get("schema_version") or "") != expected_audit_schema:
        raise ValueError("v6 unexpected audit schema")
    if audit.get("diagnostic_only") is not True:
        raise ValueError("v6 audit lost diagnostic-only boundary")
    ...
    if set(record_ids) != set(claims) or set(record_ids) != set(frozen_claims):
        raise ValueError("v6 audit/claim population mismatch")
    ...
    for record in records:
        ...
        if list(frozen_fidelity.get("reason_codes") or []) != list(
            ...
            raise ValueError("v6 semantic-fidelity replay drift")
        ...
        rows.append(
            ProspectiveAtomicAdmissibilityClaimComparisonV6(
                case_id=case_id,
                stage=stage,
...
def collect_prospective_atomic_admissibility_comparison_v6(
    freeze: ProspectiveAtomicAdmissibilityComparisonCollectorFreezeV6,
) -> ProspectiveAtomicAdmissibilityComparisonReportV6:
    all_claims = []
    case_rows = []
    ...
    for case in freeze.cases:
        ...
        if is_terminal_before_initial_vpre_status(campaign.final_status):
            if initial_stage.status != "SKIPPED_TERMINAL":
                raise ValueError("v6 terminal campaign has active initial V_pre")
            forbidden = [
            ...
            if any(path.exists() for path in forbidden):
                raise ValueError("v6 terminal campaign unexpectedly has V_pre artifacts")
            if Path(case.regeneration_reentry_report_path).exists():
                raise ValueError("v6 terminal campaign unexpectedly has reentry report")
            status: CaseAccountingStatusV6 = "TERMINAL_BEFORE_INITIAL_VPRE"
        else:
            if initial_stage.status not in {"EXECUTED", "REUSED_VALIDATED"}:
                raise ValueError("v6 nonterminal campaign lacks initial V_pre")
            initial_rows = _collect_claim_set_v6(
            ...
            if reentry_stage.status in {"EXECUTED", "REUSED_VALIDATED"}:
                ...
                if observed != expected_audits:
                    raise ValueError("v6 reentry audit population drift")
            elif reentry_path.exists():
                raise ValueError("v6 skipped reentry unexpectedly has report")
    ...
    body = {
        "schema_version": "prospective-atomic-admissibility-comparison-report-v6",
    ...
    digest = _sha256_json(body)
    return ProspectiveAtomicAdmissibilityComparisonReportV6(
        **body,
        report_id=(
            "prospective_atomic_admissibility_comparison_report_v6:"
            + digest[:20]
