...
def _write_exact_or_validate(path: Path, payload: object) -> str:
    ...
    if path.exists():
        ...
        if observed != expected:
            raise ValueError(
                "existing write-once pre-N10 re-entry artifact differs: "
                + str(path)
            )
...
class PreN10RegenerationReentryLineageV1(StrictModel):
    ...
    semantic_review_accepted: bool = False
    ...
    def validate_lineage(self) -> "PreN10RegenerationReentryLineageV1":
        ...
        if self.final_status in {
            ...
            if not self.semantic_review_accepted:
                raise ValueError(
                    "pre-N10 contract re-entry requires semantic acceptance"
                )
...
class PreN10RegenerationReentryReportV1(StrictModel):
    schema_version: Literal[
        "pre-n10-regeneration-reentry-report-v1"
    ] = "pre-n10-regeneration-reentry-report-v1"
    ...
    lineages: list[PreN10RegenerationReentryLineageV1]
    ...
    intervention_required_count: int = Field(ge=0)
    ...
    @model_validator(mode="after")
    def validate_report(self) -> "PreN10RegenerationReentryReportV1":
        ...
        if self.semantic_accepted_count != sum(
            row.semantic_review_accepted for row in self.lineages
        ):
        ...
        if self.claim_decomposition_request_count != sum(
            row.claim_decomposition_request_count
            for row in self.lineages
        ):
        ...
        if self.intervention_required_count != sum(
            row.final_status == "PRE_N10_INTERVENTION_REQUIRED"
            for row in self.lineages
        ):
            raise ValueError("intervention-required count mismatch")
        ...
        if observed_sha != expected_sha:
            raise ValueError("pre-N10 regeneration re-entry SHA mismatch")
        if observed_id != (
            "pre_n10_regeneration_reentry_v1:" + expected_sha[:20]
        ):
            raise ValueError("pre-N10 regeneration re-entry ID mismatch")
        return self


def execute_pre_n10_regeneration_reentry_v1(
    *,
    context: HypothesisContext,
    ...
    PreN10RegenerationReentryReportV1,
    ...
    rows: list[PreN10RegenerationReentryLineageV1] = []
    ...
    for source in regeneration_report.lineages:
        ...
        if (
            ...
            rows.append(
                PreN10RegenerationReentryLineageV1(
                    ...
                    regenerated_hypothesis_count=(
                        source.regenerated_hypothesis_count
                    ),
        ...
        if _sha256_file(portfolio_path) != portfolio_sha_before:
            raise ValueError(
                "regenerated portfolio mutated during semantic re-entry"
            )
        ...
        if not outcome.accepted:
            rows.append(
                PreN10RegenerationReentryLineageV1(
                    ...
                    regenerated_portfolio_id=portfolio.portfolio_id,
                    regenerated_hypothesis_count=len(portfolio.hypotheses),
                    semantic_status="SEMANTIC_REVIEW_REJECTED",
                    semantic_hard_gate_passed=True,
                    semantic_review_accepted=False,
                    semantic_runtime_invoked=True,
                    semantic_critic_llm_invoked=critic_called,
                    claim_decomposition_request_count=0,
                    final_status="SEMANTIC_REVIEW_REJECTED",
                    ready_for_n10=False,
                )
            )
            continue
    ...
    body = {
        "schema_version": "pre-n10-regeneration-reentry-report-v1",
    ...
    report = PreN10RegenerationReentryReportV1(
        **body,
        report_id=(
            "pre_n10_regeneration_reentry_v1:" + digest[:20]
        ),
        report_sha256=digest,
    )
    _write_exact_or_validate(root / "reentry.report.json", report)
