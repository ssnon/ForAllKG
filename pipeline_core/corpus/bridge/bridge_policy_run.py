...
def materialize_bridge_policy_run(
    *,
    ...
    data_root: str | Path,
    bridge_raw_output_path_fn: Callable[..., Path],
    filter_bridge_raw_chunk_fn: Callable[
        ...,
        dict[str, Any],
    ],
    build_bridge_graph_fn: Callable[..., Any],
    save_bridge_graph_fn: Callable[..., Path],
    write_bridge_tables_fn: Callable[..., Any],
    include_domain_identity_in_fingerprint: bool = True,
    publish_legacy_aliases: bool = False,
) -> dict[str, Any]:
    """
    ...
    extraction_domain = str(
        extraction_metadata.get("domain_profile_id", bridge_adapter.domain_profile_id)
    )
    extraction_adapter_id = str(
        extraction_metadata.get("bridge_adapter_id", bridge_adapter.adapter_id)
    )
    if extraction_domain != bridge_adapter.domain_profile_id:
        raise RuntimeError(
            "Bridge extraction/domain mismatch: "
            f"{extraction_domain!r} != {bridge_adapter.domain_profile_id!r}"
        )
    if extraction_adapter_id != bridge_adapter.adapter_id:
        raise RuntimeError(
            "Bridge extraction/adapter mismatch: "
            f"{extraction_adapter_id!r} != {bridge_adapter.adapter_id!r}"
        )
    ...
    policy_metadata = (
        compute_bridge_policy_run_metadata(
            ...
            bridge_policy_version=(
                bridge_adapter.policy_version
            ),
            domain_profile_id=(
                bridge_adapter.domain_profile_id
            ),
            bridge_adapter_id=(
    ...
    graph_path = save_bridge_graph_fn(
        canonical_bridge_graph,
        policy_dir
        / "bridge.graphml",
    )
