...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "critic, then create a fresh claim decomposition and re-enter the "
            "strict pre-N10 scientific contract gate. No retrieval, external "
            "novelty assessment, N9, N10, or second regeneration occurs."
        )
    )
    ...
    def semantic_runner_factory(source_hypothesis_id: str, lineage_dir: Path):
        backend = InstructorOpenAICompatibleSemanticCriticBackend(
            ...
            telemetry_context={
                "pipeline": "pre_n10_regeneration_reentry_v1",
    ...
    report, outcomes = execute_pre_n10_regeneration_reentry_v1(
        context=context,
    ...
    for row in report.lineages:
        ...
        if backend is not None:
            _write_json_exact_or_validate(
                ...
                {
                    "schema_version":
                        "pre-n10-regeneration-decomposition-prompts-v1",
    ...
    print("Pre-N10 regeneration semantic + contract re-entry complete")
    print("Report:", report.report_id)
    print("Lineages:", report.lineage_count)
    print(
        "Semantic runtime/critic-LLM/accepted/terminal:",
    ...
    print(
        ...
        "/",
        report.intervention_required_count,
    )
    print()
    for row in report.lineages:
        print(
    ...
    print("Output:", root / "reentry.report.json")
    return 0
