...
from scripts.verify_sers_fresh_c_live_discovery_recovery_v2_1_protocol_freeze import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run guarded Fresh-C C0.1C-v2.1 metadata recovery. "
    ...
    group.add_argument(
        "--confirm-live-discovery-recovery-v2-1",
        action="store_true",
    )
...
def _attempt_id(protocol_id: str, freeze_id: str) -> str:
    raw = (
        protocol_id + "\0" + freeze_id + "\0recovery-v2.1"
    ).encode("utf-8")
    return (
        "sers_fresh_c_live_discovery_recovery_attempt_v2_1:"
...
def preflight() -> int:
    ...
    print("Fresh-C C0.1C-v2.1 guarded recovery preflight")
...
def main() -> int:
    ...
    if args.preflight:
        return preflight()
    if args.confirm_live_discovery_recovery_v2_1:
        return execute()
