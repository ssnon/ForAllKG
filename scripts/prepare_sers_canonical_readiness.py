...
from dac_her.alpha4c5f1_sers_readiness import (
    audit_sers_canonical_readiness,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare a non-disclosing SERS canonical readiness lock before "
            "any future reserve consumption marker is written."
        )
    )
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ReadinessCliError(f"Expected JSON object: {path}")
    return value
...
def _paper_ids(protocol: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(raw, list) or not raw:
        raise ReadinessCliError(
            "Protocol must contain a non-empty reserve_paper_ids or paper_ids list."
        )
    paper_ids = [str(value) for value in raw]
    if len(set(paper_ids)) != len(paper_ids):
        raise ReadinessCliError("Protocol paper list contains duplicates.")
    return paper_ids
...
def main() -> int:
    ...
    audits = [
        audit_sers_canonical_readiness(paper_id, detailed=False)
        for paper_id in paper_ids
    ]
    not_ready = [row for row in audits if row["ready"] is not True]
    print("alpha4c.5f.1 SERS canonical readiness pre-consumption gate")
    ...
    if args.output is None:
        raise ReadinessCliError("--output is required with --prepare.")
    if not args.confirm_canonical_refreeze:
        raise ReadinessCliError(
            "--confirm-canonical-refreeze is required with --prepare. "
            "No extraction LLM is called; only eligible canonical migration "
            "may occur."
        )
    if any(not row["refreeze_eligible"] for row in not_ready):
        blocked = [
            row["paper_id"]
            for row in not_ready
        ...
        raise ReadinessCliError(
            "Non-refreeze-eligible readiness failures: " + ", ".join(blocked)
        )

    output_path = ROOT / args.output
    if output_path.exists():
        raise ReadinessCliError(
            f"Refusing to replace existing readiness lock: {output_path}"
        )
