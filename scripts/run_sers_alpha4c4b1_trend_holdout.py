...
def verify_protocol(
    ...
    _require_equal("phase", protocol.get("phase"), "alpha4c.4b.1")
    _require_equal(
        "state",
        protocol.get("state"),
    ...
    canonical_policy = protocol.get("canonical_input_policy", {})
    _require_equal(
        "pre-existing resolution decisions policy",
        canonical_policy.get(
            "preexisting_resolution_decisions_allowed"
        ),
        True,
    )
    _require_equal(
        "post-lock resolution changes allowed",
        canonical_policy.get(
            "post_snapshot_resolution_changes_allowed"
        ),
        False,
    )
    _require_equal(
        "resolution decisions hash lock",
        canonical_policy.get(
            "resolution_decisions_hash_locked_at_campaign_start"
        ),
        True,
    )
    ...
    if len(holdout) != 10 or len(holdout) != len(set(holdout)):
        raise FrozenTrendHoldoutError(
            "alpha4c.4b.1 must contain exactly 10 unique frozen holdout papers."
        )
...
def _resolution_decisions_snapshot(
    paper_root: Path,
) -> dict[str, Any]:
    """Freeze pre-existing resolution provenance without interpreting it.

    A decisions.jsonl file may be part of the pre-existing curated canonical
    substrate. Presence alone is not holdout contamination. What is forbidden
    is changing/creating/removing that file after the alpha4c.4b.1 canonical
    input lock is created.
    """
...
def snapshot_canonical_inputs(
    ...
    for paper_id in protocol["holdout_papers"]:
        ...
        resolution_decisions = _resolution_decisions_snapshot(
            paper_root
        )
    ...
    if missing:
        raise FrozenTrendHoldoutError(
            "Frozen alpha4c.4b.1 canonical KG substrate is not ready for "
...
def _freeze_or_verify_canonical_input_lock(
    *,
    protocol: Mapping[str, Any],
    protocol_path: Path,
    ...
    lock_path = evaluation_root / "canonical_input_lock.json"
    payload = {
        "phase": "alpha4c.4b.1",
        ...
        "lock_role": (
            "Freeze canonical GraphML plus pre-existing resolution "
            "decision-file presence/content before Trend holdout outputs."
        ),
    }

    if lock_path.exists():
        locked = _read_json(lock_path)
        locked_comparable = dict(locked)
        locked_comparable.pop("created_at_utc", None)
        _require_equal(
        ...
        payload_to_write["created_at_utc"] = _now()
        _atomic_json(lock_path, payload_to_write)
...
def _verify_canonical_snapshot_unchanged(
    ...
    for paper_id, record in snapshot.items():
        ...
        resolution = record.get("resolution_decisions", {})
        resolution_path = PROJECT_ROOT / str(
            resolution.get("path", "")
        )
        was_present = bool(resolution.get("present", False))
        is_present = resolution_path.exists()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the alpha4c.4b.1 frozen unseen SERS Trend holdout. "
...
def main() -> int:
    ...
    print("alpha4c.4b.1 frozen protocol: PASS")
    print(
        "Holdout:",
    ...
    resolution_files = sum(
        bool(record.get("resolution_decisions", {}).get("present"))
        for record in canonical_snapshot.values()
    )
    resolution_rows = sum(
        int(
            record.get("resolution_decisions", {}).get(
                "nonempty_line_count", 0
            )
        )
        for record in canonical_snapshot.values()
    ...
    print(
        "Pre-existing resolution decision files:",
        resolution_files,
        "(rows:",
        resolution_rows,
        ")",
    ...
    print(
        "Resolution provenance policy: pre-existing files are allowed "
        "as frozen curated substrate; post-lock mutation is forbidden."
    )

    input_lock_path = _freeze_or_verify_canonical_input_lock(
        protocol=protocol,
        protocol_path=protocol_path,
        protocol_audit=protocol_audit,
        canonical_snapshot=canonical_snapshot,
    )
    print("Canonical input lock:", input_lock_path)

    if args.preflight_only:
        print(
            "No projection/corpus/Trend scientific output was generated "
            "in --preflight-only mode. Canonical/resolution provenance "
            "was hash-locked for the real run."
        )
        return 0
    ...
    if manifest_path.exists():
        ...
        _require_equal(
            "campaign canonical input lock",
            manifest.get("canonical_input_lock"),
            str(input_lock_path.relative_to(PROJECT_ROOT)),
        )
        _require_equal(
        ...
        manifest = {
            ...
            "frozen_implementation_blobs":
                protocol_audit["implementation_blobs"],
            "canonical_snapshot": canonical_snapshot,
            "canonical_input_lock": str(
                input_lock_path.relative_to(PROJECT_ROOT)
            ),
            "llm_calls_performed_by_runner": False,
            "bridge_used": False,
            "projection_mode": protocol["mode"],
    ...
    if args.dry_run:
        ...
        _atomic_json(manifest_path, manifest)
        print("alpha4c.4b.1 dry-run: COMPLETE")
    ...
    print()
    print("alpha4c.4b.1 frozen Trend holdout verdict:", report["verdict"])
    print(
