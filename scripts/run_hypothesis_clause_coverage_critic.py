...
from dac_her.hypothesis_clause_coverage import (
    HypothesisClauseCoverageRuntime,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "PS3 evidence-blind clause decomposition followed by full-set "
            "grounding / inferential-leap audit."
        )
    )
    ...
    parser.add_argument("--ps2-report", type=Path, default=None)
...
def main() -> int:
    ...
    ps2 = None
    if args.ps2_report is not None:
        ps2 = PremiseRoleNecessityReport.model_validate_json(
            args.ps2_report.read_text(encoding="utf-8")
        )
    ...
    report, prompts = HypothesisClauseCoverageRuntime(
        backend
    ).run(
        context,
        portfolio,
    ...
    output_path = Path(
        ...
        + ".hypothesis_clause_coverage.ps3.json"
    )
    ...
    if args.save_prompts:
        ...
        for hypothesis_id, stage, prompt in prompts:
            filename = (
    ...
    print("PS3 full-set clause coverage / inferential-leap audit")
    ...
    print(
        "Invalid clause quotes / support refs:",
        f"{report.invalid_clause_quote_count}/"
    ...
    for card in report.cards:
        ...
        print(" overall:", card.overall_verdict)
        ...
        if card.ps2_snapshot is not None:
            print(
                " PS2 premise verdicts:",
                card.ps2_snapshot.premise_verdicts,
            )
