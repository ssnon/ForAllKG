...
from scripts.verify_sers_fresh_c_live_discovery_recovery_v2_3_protocol_freeze import (
...
def main() -> int:
    ...
    if (run_dir / "DISCOVERY_RECOVERY_FAILED.json").exists():
        raise RuntimeError("v2.3 failed epoch exists; success verification forbidden.")
    ...
    if manifest.get("protocol_id") != p.protocol_id:
        raise ValueError("v2.3 result protocol mismatch.")
    if manifest.get("successful_provider_query_executions") != 8:
        raise ValueError("v2.3 does not have 8/8 provider success.")
    if manifest.get("fresh_identity_queue_count", 0) < 25:
        raise ValueError("v2.3 fresh queue below target.")
    ...
    if manifest.get("semantic_read_performed") is not False:
        raise ValueError("v2.3 performed semantic read.")
    if manifest.get("automatic_c0_1d_transition_authorized") is not False:
        raise ValueError("v2.3 auto-authorized C0.1D.")
    ...
    for filename, field in (
        ...
        if sha256_file(run_dir / filename) != manifest.get(field):
            raise ValueError(f"v2.3 file SHA drifted: {filename}")

    if diagnostics.get("fresh_reserve_c_consumed") is not False:
        raise ValueError("v2.3 diagnostics consumed Fresh C.")
    if diagnostics.get("semantic_read_performed") is not False:
        raise ValueError("v2.3 diagnostics semantic read.")
    if diagnostics.get("llm_calls") != 0:
        raise ValueError("v2.3 diagnostics used LLM.")
    ...
    for i, row in enumerate(records, start=1):
        if row["canonical_id"] in historical:
            raise ValueError("Historical identity leaked into v2.3 queue.")
        if row["rank"] != i:
            raise ValueError("v2.3 rank sequence drifted.")
        if row["score_sha256"] != _score(row["canonical_id"]):
            raise ValueError("v2.3 blind score drifted.")
    ...
    if queue_ids != locator_ids:
        raise ValueError("v2.3 locator/queue set mismatch.")
    if complete.get("run_id") != manifest.get("run_id"):
        raise ValueError("v2.3 COMPLETE run ID mismatch.")

    print("Fresh-C C0.1C-v2.3 recovery result verifier")
