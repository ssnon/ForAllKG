...
def main() -> int:
    ...
    if spec_issues:
        print("hypothesis-novelty synthesis DEV verification: FAIL")
        for issue in spec_issues:
            print(" -", issue)
        return 2

    report_path = run_root / "hypothesis_novelty_synthesis_report.json"
    audit_path = run_root / "human_hypothesis_novelty_audit.md"
    ...
    recomputed = compute_hypothesis_synthesis(
        plan=source["plan"],
    ...
    if report.get("status_counts") != expected_counts:
        issues.append("status-count mismatch")

    expected_audit = render_manual_audit(recomputed)
    ...
    if report.get("scientific_novelty_status_outcome") != (
        ...
        issues.append("scientific novelty status overclaimed")
    ...
    required_false = (
        ...
        "claim_decomposition_recomputed",
        "canonicalization_recomputed",
    ...
    print("hypothesis-novelty synthesis DEV verification: PASS")
