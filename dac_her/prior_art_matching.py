...
_REACTION_PATTERNS: dict[str, tuple[re.Pattern[str], ...]] = {
    "HER": (
        re.compile(r"\bher\b", re.I),
        re.compile(r"hydrogen\s+evolution", re.I),
    ),
    "OER": (
        re.compile(r"\boer\b", re.I),
        re.compile(r"oxygen\s+evolution", re.I),
    ),
    "ORR": (
        re.compile(r"\borr\b", re.I),
        re.compile(r"oxygen\s+reduction", re.I),
    ),
    "CO2RR": (
        re.compile(r"\bco2rr\b", re.I),
        re.compile(r"co2\s+(?:electro)?reduction", re.I),
        re.compile(r"carbon\s+dioxide\s+(?:electro)?reduction", re.I),
    ),
    "NRR": (
        re.compile(r"\bnrr\b", re.I),
        re.compile(r"nitrogen\s+reduction", re.I),
        re.compile(r"nitrogen\s+fixation", re.I),
    ),
}
...
def _reaction_domain_relevance(claim_text: str, document: str) -> float:
    claim_domains = _reaction_domains(claim_text)
    doc_domains = _reaction_domains(document)
    if not claim_domains:
        return 0.5
    if claim_domains & doc_domains:
        score = 1.0
    elif doc_domains:
        score = 0.05
    else:
    ...
    claim_electro = bool(
        re.search(r"electrocatal|exchange\s+current|\bher\b", claim_text, re.I)
    )
    doc_photo = bool(re.search(r"photocatal|photoelectro", document, re.I))
    doc_electro = bool(re.search(r"electrocatal|exchange\s+current", document, re.I))
    if claim_electro and doc_photo and not doc_electro:
        score *= 0.55
    return max(0.0, min(1.0, score))


_SCOPE_PATTERNS: dict[str, tuple[re.Pattern[str], ...]] = {
    "nitrogen_coordination": (
        re.compile(r"nitrogen.{0,35}coordin", re.I),
        re.compile(r"coordin.{0,35}nitrogen", re.I),
        re.compile(r"\bmn\s*4\b", re.I),
        re.compile(r"\bm[\-–—]?n\s*4\b", re.I),
        re.compile(r"\bn\s*[23456]\b", re.I),
    ),
    "dual_atom": (
        re.compile(r"dual[ -]?atom", re.I),
        re.compile(r"diatomic", re.I),
        re.compile(r"\bdimer", re.I),
        re.compile(r"metal[ -]?pair", re.I),
        re.compile(r"paired\s+metal", re.I),
    ),
    "d_band": (
        re.compile(r"d[ -]?band", re.I),
    ),
    "hydrogen_adsorption": (
        re.compile(r"hydrogen\s+(?:adsorption|binding)", re.I),
        re.compile(r"\bdelta\s*g.{0,8}h\b", re.I),
    ),
    "exchange_current": (
        re.compile(r"exchange\s+current", re.I),
    ),
}
...
def _catalyst_scope_relevance(claim_text: str, document: str) -> float:
    claim_features = _scope_features(claim_text)
    if not claim_features:
        return 0.5
    doc_features = _scope_features(document)
...
def _strong_scope_compatibility(
    ...
    *,
    ...
    reaction = _reaction_domain_relevance(claim_text, document)
    scope = _catalyst_scope_relevance(claim_text, document)
    claim_features = _scope_features(claim_text)
    doc_features = _scope_features(document)
    reasons: list[str] = []

    if reaction < min_reaction:
        reasons.append("reaction_domain_mismatch")

    # Distinctive structural qualifiers are hard requirements for a strong
    # direct/conflicting match. This is what keeps a general AuPt alloy d-band
    # counterexample contextual rather than a direct contradiction of an
    # N-coordination-specific hypothesis.
    for critical in ("nitrogen_coordination", "dual_atom"):
        if critical in claim_features and critical not in doc_features:
            reasons.append(f"missing_critical_scope:{critical}")

    if scope < min_scope:
        reasons.append("low_catalyst_scope_overlap")

    compatible = not reasons
    return compatible, reaction, scope, reasons
...
class PriorArtRanker:
    ...
    def rank(
        ...
        for work, document, vector in zip(candidate_rows, documents, encoded, strict=True):
            ...
            reaction = _reaction_domain_relevance(claim.text, document)
            scope = _catalyst_scope_relevance(claim.text, document)
            score = max(
                0.0,
                min(
                    1.0,
...
class ClaimPriorArtCompiler:
    ...
    def compile(
        ...
        if unknown:
            raise ValueError(
                "prior-art reviewer returned unknown/unranked work IDs: " + ",".join(unknown)
            )
        ...
        for row in draft.matches:
            ...
            document = "\n".join(x for x in [work.title, work.abstract or ""] if x)
            compatible, reaction, scope, scope_reasons = _strong_scope_compatibility(
                claim.text,
                document,
                min_reaction=self.min_reaction_for_conflict,
