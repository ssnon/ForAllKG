...
def resolve_semantic_stop_max_depth(
    *,
    ...
    """Resolve the semantic-stop depth while preserving old explicit overrides.

    v2.4.7 gives semantic-stop a larger default depth because a mandatory
    waypoint makes valid paths structurally longer. If the caller explicitly
    supplied the legacy global ``--max-depth`` and did not supply the new
    semantic-stop-specific option, the explicit legacy value is preserved.
    """
...
def guard_semantic_stop_ablation(
    ...
    *,
    ...
    Disabling EndpointPairSelector is intentionally an ablation path, not the
    production retrieval path. Without a guard, ``source × target × waypoint``
    can grow to thousands of expensive semantic-stop traversals. We keep the
    highest-ranked mapper candidates from both endpoint sides and choose a
    symmetric per-side cap so the estimated triple count cannot exceed the
    requested budget.
    """
