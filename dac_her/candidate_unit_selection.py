...
class CandidateUnitSelector:
    ...
    def _route_diagnostics(
        ...
        *,
        ...
        left_mech = any(_edge_is_mechanistic(step) for step in left_edges) or any(
            node in self.graph and _is_mechanism_node(node, dict(self.graph.nodes[node]))
            for node in left_nodes
        )
        right_mech = any(_edge_is_mechanistic(step) for step in right_edges) or any(
            node in self.graph and _is_mechanism_node(node, dict(self.graph.nodes[node]))
        ...
        for node in confirmed_nodes[1:-1]:
            ...
            if _is_alignment_node(attrs):
                continue
            generic_flags.append(_is_generic_node(node, attrs))
        ...
        scaffold_count = sum(_edge_is_scaffold(step) or edge_is_alignment(step) for step in confirmed_steps)
        ...
        for node in confirmed_nodes:
            if node not in self.graph:
                continue
            attrs = dict(self.graph.nodes[node])
            if _normalized_type(attrs) == "REACTION":
