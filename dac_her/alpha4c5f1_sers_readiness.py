...
ALPHA4C5F1_SERS_READINESS_ID = (
    "sers_canonical_readiness_v1_alpha4c5f1"
)
SERS_DOMAIN_PROFILE_ID = "sers_au_ag"
SERS_CONFIG = "configs/papers_sers_au_ag.yaml"


# Only stale/missing canonical representation may be deterministically rebuilt.
# A domain mismatch or a rejected/invalid Strict source is a hard stop.
...
def _canonical_path(paper_id: str) -> Path:
    return DATA_ROOT / "extracted" / paper_id / f"{paper_id}.graphml"
...
def _snapshot(paper_id: str, *, detailed: bool) -> dict[str, Any]:
    ...
    row["canonical_path"] = _relative(_canonical_path(paper_id))
    return row


def audit_sers_canonical_readiness(
    paper_id: str,
    *,
    detailed: bool = False,
) -> dict[str, Any]:
    """Audit Strict-source eligibility plus canonical structural readiness."""
    source = resolve_strict_source(paper_id)
    canonical = _snapshot(paper_id, detailed=detailed)
    return {
        "paper_id": paper_id,
        "readiness_id": ALPHA4C5F1_SERS_READINESS_ID,
        "strict_quality_status": source["extraction_quality"][
...
def prepare_sers_canonical_paper(
    paper_id: str,
    *,
    allow_refreeze: bool,
) -> dict[str, Any]:
    """Make one canonical graph ready from already-frozen Strict inputs.

    This function never invokes extraction. It may call build_paper_graph only
    when the canonical representation is missing/stale or violates the frozen
    Measurement XOR invariant. Manual resolution decisions and Strict source
    bytes are verified unchanged across that deterministic rebuild.
    """
    source = resolve_strict_source(paper_id)
    ...
    before_decisions_snapshot = snapshot_optional(ROOT, decisions_path)
    ...
    if before["ready"] is not True:
        ...
        if not reasons.issubset(REFREEZE_ELIGIBLE_REASONS):
            raise CanonicalReadinessError(
                f"{paper_id}: readiness failure is not eligible for "
                f"canonical refreeze: {sorted(reasons)!r}"
            )
        if not allow_refreeze:
            raise CanonicalReadinessError(
                f"{paper_id}: canonical refreeze required but not allowed: "
                f"{sorted(reasons)!r}"
            )
        ...
        if source["attempt_id"]:
            command.extend(["--attempt-id", source["attempt_id"]])
        ...
        if result.returncode != 0:
            raise CanonicalReadinessError(
                f"{paper_id}: deterministic canonical refreeze failed "
    ...
    if after["ready"] is not True:
        raise CanonicalReadinessError(
            f"{paper_id}: canonical is still not ready after preparation: "
            f"{after['readiness_issues']!r}"
        )
    if after["measurement_xor_issue_count"] != 0:
        raise CanonicalReadinessError(
            f"{paper_id}: nonzero Measurement XOR count after preparation."
        )
    ...
    if before_manual != after_manual:
        raise CanonicalReadinessError(
            f"{paper_id}: manual resolution decisions changed during "
            "canonical preparation."
        )
    after_decisions_snapshot = snapshot_optional(ROOT, decisions_path)
    ...
    return {
        ...
        "resolution_decisions_before": before_decisions_snapshot,
        "resolution_decisions": after_decisions_snapshot,
        "new_extraction_llm_calls": 0,
    }


def prepare_sers_canonical_readiness_lock(
    *,
    ...
    for paper_id in ordered:
        records[paper_id] = prepare_sers_canonical_paper(
            paper_id,
