    ...
    for node_id, attrs in graph.nodes(data=True):
        ...
        if any(marker in text for marker in calculation_markers):
            rows.append({
                ...
                "message": (
                    f"Experiment {node_id!r} is calculation/simulation-like; "
                    "SERS DDA/FDTD/FEM/BEM/DFT/TDDFT work should be Calculation."
                ),
            })
            continue

        experiment_family = str(attrs.get("experiment_family", "")).lower()
        experiment_type = str(attrs.get("experiment_type", "")).lower()
        if (
            experiment_family == "synthesis"
            or experiment_type.startswith("synthesis")
            or any(marker in text for marker in synthesis_markers)
