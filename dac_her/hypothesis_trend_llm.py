...
from dac_her.hypothesis_trend_prompt import TrendAwareHypothesisPrompt
from dac_her.llm_telemetry import run_instructor_structured_call


@dataclass(frozen=True)
class TrendAwareHypothesisDraftGeneration:
...
@runtime_checkable
class TrendAwareHypothesisDraftBackend(Protocol):
    backend_name: str
    model_name: str
...
class InstructorOpenAICompatibleTrendHypothesisBackend:
    """Structured alpha4c.5d Trend-aware hypothesis draft backend."""

    backend_name = "instructor_openai_compatible_trend_hypothesis"
    ...
    def _get_client(self) -> Any:
        ...
        try:
            ...
            raise RuntimeError(
                "Trend-aware Hypothesis Maker requires installed 'openai' and 'instructor' packages."
            ) from exc

        mode = getattr(instructor.Mode, self.instructor_mode, None)
        if mode is None:
            available = sorted(
                name for name in dir(instructor.Mode) if name.isupper()
            )
            raise ValueError(
        ...
        *,
        ...
        context = {
            **self.telemetry_context,
            "pipeline": "trend_aware_hypothesis_maker",
            "stage": stage,
            "call_kind": stage,
        }
        draft, event = run_instructor_structured_call(
            ...
            response_model=TrendAwareHypothesisPortfolioDraft,
            messages=messages,
        ...
        if not isinstance(draft, TrendAwareHypothesisPortfolioDraft):
            draft = TrendAwareHypothesisPortfolioDraft.model_validate(draft)
        return TrendAwareHypothesisDraftGeneration(
            draft=draft,
    ...
    def generate(
        ...
        return self._call(
            ...
            stage="generation",
            semantic_components={
                "trend_exposure_id": prompt.exposure_id,
                "trend_exposure_sha256": prompt.exposure_sha256,
            },
        )
