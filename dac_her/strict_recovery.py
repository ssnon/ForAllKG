...
def _is_reserved_collection_domain_violation(error: Exception) -> bool:
    message = str(error)
    return (
        isinstance(error, ValueError)
        and "Extraction-domain vocabulary violation" in message
        and "reserved structured node type(s)" in message
        and "dedicated top-level collection" in message
    )
...
def extract_one_chunk(
    *,
    ...
    last_domain_rejected_draft: KnowledgeGraphDraft | None = None
    last_domain_gate_error: ValueError | None = None
    ...
    for generation_attempt in range(policy.max_generation_attempts):
        ...
        try:
            ...
            try:
                ...
                if _is_reserved_collection_domain_violation(domain_error):
                    last_domain_rejected_draft = draft
                    last_domain_gate_error = domain_error
                raise
            ...
            # A reserved structured node placed in entities[] is not a
            # scientific-domain disagreement; it is a recoverable collection
            # placement error in an otherwise parseable draft. Give exactly
            # one targeted complete-draft repair before terminal quarantine.
            if (
                last_domain_rejected_draft is not None
                and last_domain_gate_error is not None
                and _is_reserved_collection_domain_violation(error)
            ):
                recovery_llm = OpenRouterLLM(
                    model=model,
                    provider=provider,
                    reproducible=False,
                    zdr=True,
                ...
                try:
                    ...
                    recovered_draft = _domain_gate(
                        recovered_draft,
                        extraction_adapter=extraction_adapter,
                    )

                    recovery_usage = dict(
                        recovery_llm.last_call_metadata or {}
                    )
                    recovery_usage["max_completion_tokens"] = (
                        policy.max_completion_tokens
                    )
                    recovery_usage["call_kind"] = "domain_gate_recovery"
                    recovery_usage["domain_gate_recovery_attempt"] = 0
                    attempt_usages.append(recovery_usage)

                    _write_json(
                        candidate_dir
                        / f"{safe_chunk_id}__domain_gate_recovery_0.json",
                        recovered_draft.model_dump(),
                    )
                    ...
                    recovery_usage["call_kind"] = "domain_gate_recovery"
                    recovery_usage["domain_gate_recovery_attempt"] = 0
                    recovery_usage["error_type"] = (
                        type(recovery_error).__name__
                    )
                    recovery_usage["error_message"] = str(recovery_error)
                    attempt_usages.append(recovery_usage)
            ...
            _write_json(
                quarantine_path,
                {
                    ...
                    "error_message": str(error),
                    "debug_path": str(raw_debug_path),
                    "domain_gate_recovery_attempted": (
                        last_domain_rejected_draft is not None
                    ),
                },
            )
