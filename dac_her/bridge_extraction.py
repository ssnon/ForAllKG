...
from dac_her.bridge_recovery import (
...
def extract_bridge_chunk(
    *,
    ...
    """Backward-compatible extraction + deterministic filtering wrapper."""
    ...
    raw_output_path = Path(
        str(raw_record["raw_output_path"])
    )
    raw_result = (
        BridgeChunkGraph.model_validate_json(
            raw_output_path.read_text(
    ...
    filtered_record = filter_bridge_raw_chunk(
    ...
    return {
        **filtered_record,
        ...
        "validation_repairs": raw_record.get(
            "validation_repairs",
            0,
        ),
        "normalization_count": raw_record.get(
            "normalization_count",
            0,
        ),
        "repaired_candidate_count": raw_record.get(
            "repaired_candidate_count",
            0,
        ),
        "quarantined_candidate_count": raw_record.get(
            "quarantined_candidate_count",
            0,
        ),
        "recovery_path": raw_record.get(
            "recovery_path",
            "",
        ),
        "quarantine_path": raw_record.get(
            "quarantine_path",
            "",
        ),
    }
