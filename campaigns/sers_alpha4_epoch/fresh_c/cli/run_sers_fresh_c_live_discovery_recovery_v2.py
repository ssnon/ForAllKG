...
from campaigns.sers_alpha4_epoch.fresh_c.fresh_c_live_discovery_recovery_v2 import (
    ...
    make_transport_diagnostics_payload,
...
from campaigns.sers_alpha4_epoch.fresh_c.cli.verify_sers_fresh_c_live_discovery_recovery_v2_protocol_freeze import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run guarded Fresh-C C0.1C-v2 metadata recovery. "
    ...
    group.add_argument(
        "--confirm-live-discovery-recovery-v2",
        action="store_true",
    )
...
def _attempt_id(protocol_id: str, freeze_id: str) -> str:
    raw = (
        protocol_id + "\0" + freeze_id + "\0recovery-v2"
    ).encode("utf-8")
    return (
        "sers_fresh_c_live_discovery_recovery_attempt_v2:"
...
def preflight() -> int:
    ...
    print("Fresh-C C0.1C-v2 guarded recovery preflight")
...
def execute() -> int:
    ...
    started = {
        "schema_version": (
            "sers-fresh-c-live-discovery-recovery-started-v2"
        ),
    ...
    try:
        ...
        diagnostics = make_transport_diagnostics_payload(
            protocol=protocol,
        ...
        run_body: dict[str, Any] = {
            "schema_version": (
                "sers-fresh-c-live-discovery-recovery-run-v2"
            ),
            "protocol_id": protocol.protocol_id,
            "protocol_sha256": protocol.protocol_sha256,
            "freeze_id": freeze["freeze_id"],
        ...
        run_body["run_id"] = (
            "sers_fresh_c_live_discovery_recovery_run_v2:"
            + identity_sha[:20]
        )
        ...
        _atomic_json(
            ...
            {
                "schema_version": (
                    "sers-fresh-c-live-discovery-recovery-complete-v2"
                ),
                "attempt_id": attempt_id,
        ...
        _atomic_json(
            ...
            {
                "schema_version": (
                    "sers-fresh-c-live-discovery-recovery-failed-v2"
                ),
                "attempt_id": attempt_id,
...
def main() -> int:
    ...
    if args.preflight:
        return preflight()
    if args.confirm_live_discovery_recovery_v2:
        return execute()
