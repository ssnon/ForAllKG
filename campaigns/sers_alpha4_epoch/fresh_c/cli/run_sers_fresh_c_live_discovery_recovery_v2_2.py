...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument(
        "--confirm-live-discovery-recovery-v2-2",
        action="store_true",
    )
...
def _attempt_id(protocol_id: str, freeze_id: str) -> str:
    raw = (
        protocol_id + "\0" + freeze_id + "\0recovery-v2.2"
    ).encode("utf-8")
    return (
        "sers_fresh_c_live_discovery_recovery_attempt_v2_2:"
...
def _preflight(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "recovery_parent_attempt_id": EXPECTED_V21_FAILED_ATTEMPT_ID,
...
def execute() -> int:
    ...
    try:
        ...
        diagnostics = make_transport_diagnostics_payload_v2_2(
            protocol_id=protocol.protocol_id,
            parent_attempt_id=EXPECTED_V21_FAILED_ATTEMPT_ID,
...
def main() -> int:
    ...
    if args.preflight:
        return preflight()
    if args.confirm_live_discovery_recovery_v2_2:
        return execute()
