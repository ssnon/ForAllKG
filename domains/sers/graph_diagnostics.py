...
def collect_sers_graph_diagnostics(
    graph: nx.MultiDiGraph,
    graph_adapter: GraphDomainAdapter,
) -> dict[str, Any]:
    if graph_adapter.domain_profile_id != "sers_au_ag":
        raise ValueError(
            "SERS graph diagnostics require domain_profile_id='sers_au_ag'"
        )
    ...
    return {
        ...
        "node_role_issues": node_role_diagnostics(
            graph,
            domain_profile_id=domain_profile_id,
        ),
        ...
        "component_bridge_candidates": component_bridge_candidates,
    }
